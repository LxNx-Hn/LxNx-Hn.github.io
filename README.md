...
프로젝트는 `소개 → Tech Stack → 내가 맡은 부분 → 핵심 문제·판단 → 프로젝트 결과 → 관련 커밋` 순서로 정리합니다. 단순 버그 수정이나 일반적인 CRUD 이슈는 문제 사례로 올리지 않고, 프로젝트 목적 때문에 직접 기준을 정해야 했던 설계·평가·실험 문제만 남깁니다. 문제 사례가 많은 프로젝트는 접이식 case study로 두어 개요는 빠르게 보고 필요할 때 구현·디버깅 과정을 깊게 확인할 수 있게 했습니다. 사실과 작업 범위는 공개 저장소의 README와 대표 커밋 diff를 기준으로 확인하며, 본인 작성 커밋과 AI 공동작성 커밋을 구분합니다. 재현할 수 없는 수치나 프로젝트 전체를 단독 구현했다는 표현은 포함하지 않습니다.
