...
프로젝트는 `소개 → Tech Stack → 내가 맡은 부분 → 문제·원인·시도·해결·결과 → 관련 커밋` 순서로 정리합니다. 사실과 작업 범위는 공개 저장소의 README와 대표 커밋 diff를 기준으로 확인하며, 본인 작성 커밋과 AI 공동작성 커밋을 구분합니다. 재현할 수 없는 수치나 프로젝트 전체를 단독 구현했다는 표현은 포함하지 않습니다.
