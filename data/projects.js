export const projects = [
  {
    ...
    "contributions": [
      ...
      "LLM 평가를 그대로 정답으로 쓰지 않고 고정 rubric으로 6,822개 bootstrap label을 다시 만들어 6개 프로필별 XGBRanker baseline을 학습·검증했습니다.",
      "화면에 보여줄 경로 수와 모델이 비교할 내부 candidate pool을 분리해 개인화 전에 후보가 잘리지 않도록 했습니다.",
    ...
    "cases": [
      {
        ...
        "approach": "먼저 실제 경로 후보를 6개 프로필 관점에서 LLM으로 1차 평가해 점수와 판단 근거를 모았습니다. 이후 결과를 그대로 학습 라벨로 쓰지 않고, 어떤 항목이 반복해서 영향을 주는지와 점수 분포를 다시 확인해 프로필별 평가 기준을 정리했습니다.",
        ...
        "result": "프로필별 OD holdout에서 NDCG@3 0.9166~0.9596을 확인했습니다. 다만 이 수치는 LLM과 고정 rubric을 거쳐 만든 초기 평가 기준을 재현하는 baseline 성능이며 실제 사용자 선호 검증 결과로 해석하지 않았습니다."
      },
      {
        "title": "개인화 전 후보 보존",
